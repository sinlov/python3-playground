...
"""
    ...
    @staticmethod
    def init_logger(
        tag=str, log_dir='logs', prefix='plog', postfix='log', level=logging.DEBUG
    ):
...
""".format(cli_version)
    ...
    def check_args_size_not_zero(self):
        if len(self._parse_args) == 0:
            print(
                'Error input args size is zero {0}'.format(OptDefClass.enter_error_info)
            )
            raise Exception()
...
if __name__ == '__main__':
    ...
    if options.c_clean:
        PLog.log_writer(
            'now clean flag {0} force flag {1}'.format(options.c_clean, options.force),
            'd',
        )
    ...
    if not os.path.exists(config_file_path):
        PLog.log_writer(
            '-> load config_file_path not exist: {0}'.format(config_file_path), 'e'
        )
