...
""".format(
    ...
    def __init__(self):
        ...
        self_parser.add_option(
            "-v", dest="v_verbose", action="store_true", help="[-|+] see verbose", default=False
        )
        self_parser.add_option(
            "--no-log",
            dest="no_log",
    ...
    def check_args_size_not_zero(self):
        if len(self._parse_args) == 0:
            print("Error input args size is zero {0}".format(OptDefClass.enter_error_info))
            raise Exception()
...
if __name__ == "__main__":
    ...
    if options.c_clean:
        PLog.log_writer(
            "now clean flag {0} force flag {1}".format(options.c_clean, options.force), "d"
        )
