...
git_target_from = 'gitea.foo.com'
git_target_to = 'gitea.bar.com'
...
def start_migration_by_folder(folder, from_git_host, to_git_host) -> int:
    ...
    for want_git_dir in os.listdir(folder):
        ...
        if not os.path.exists(git_hide_path) or os.path.isfile(git_hide_path):
            print(
                f'-> want migration folder not git work path: {want_migration_folder}'
            )
            continue
        # print(f'~> start migration path: {want_migration_folder}')
        code, out, err = Exec.exec(
            'git config --get remote.origin.url', want_migration_folder
        )
        if code != 0:
            print(f'err: {err}')
            continue
        ...
        if from_git_host not in now_url:
            print(
                f'~> pass migration path: {want_migration_folder} remote.origin.url: {now_url}'
            )
            continue
        ...
        if m_code != 0:
            print(
                f'~> change remote.origin.url err at {want_migration_folder} to {new_url}'
            )
            continue
        cnt += 1
