...
def start_migration_by_folder(folder, from_git_host, to_git_host) -> int:
    ...
    for want_git_dir in os.listdir(folder):
        ...
        m_code, m_out, m_err = Exec.exec(f'git config remote.origin.url {new_url}', want_migration_folder)
        if m_code != 0:
...
if __name__ == '__main__':
    ...
    self_parser.add_option('-f', '--from', dest='from_git_host', type="string",
                           help=f"want migration from host, default [ {git_target_from} ]",
                           default=git_target_from,
                           metavar=git_target_from)
    self_parser.add_option('-t', '--to', dest='to_git_host', type="string",
    ...
    m_count = 0
    m_count += start_migration_by_folder(migration_folder, _parse_opt.from_git_host, _parse_opt.to_git_host)
    print()
